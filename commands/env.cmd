...
[[ ${WARDEN_ELASTICSEARCH} -eq 1 && ${WARDEN_ELASTICHQ:=1} -eq 1 ]] \
...
if { [[ "${WARDEN_PARAMS[0]}" == "up" ]] || [[ "${WARDEN_PARAMS[0]}" == "start" ]]; } \
    ...
    && [[ $($WARDEN_BIN env ps -q ${WARDEN_CONTAINER_FOR_SYNC:-php-fpm}) ]] \
    && [[ $(docker container inspect "$($WARDEN_BIN env ps -q ${WARDEN_CONTAINER_FOR_SYNC:-php-fpm})" --format '{{ .State.Status }}') = "running" ]] \
    && [[ $($WARDEN_BIN env ps -q ${WARDEN_CONTAINER_FOR_SYNC:-php-fpm}) = $($WARDEN_BIN sync list | grep -i 'URL: docker' | awk -F'/' '{print $3}') ]]
then
